...
describe('listing writer endpoint', () => {
  ...
  it('returns deterministic copy for valid permits', async () => {
    await request(app).post('/permits/check').send({ trakheesi_number: '11112222' });
  ...
  it('blocks requests when permit is invalid or expired', async () => {
    const invalidResponse = await request(app).post('/nlp/listing-writer').send({
    ...
    const expiredResponse = await request(app).post('/nlp/listing-writer').send({
